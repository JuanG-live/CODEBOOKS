...
int UserRegistration(const char fileName[]){
    ...
    do {
        salirRegistro = CapturarPalabra(reader.surName, 36, 6, SURNAME, 0);
    ...
    while (validaIngreso == 0 && salirRegistro != ESC) {
        salirRegistro = CapturarPalabra(reader.name, 36, 8, NAME, 0);
    ...
    if (salirRegistro != ESC){
        salirRegistro = CapturarPalabra(reader.phone, 36, 10, PHONE, 0);
    }
    if (salirRegistro != ESC){
        salirRegistro = CapturarPalabra(reader.address, 36, 12, ADDRESS, 0);
    }

    validaIngreso = 0;
    int validaMail = 0;
    while (validaIngreso == 0 && salirRegistro != ESC){
        salirRegistro = CapturarPalabra(reader.mail, 36, 14, MAIL, 0);
    ...
    while (validaIngreso == 0 && salirRegistro != ESC){
        ...
        while (validaPassword == 0 && salirRegistro != ESC){
            salirRegistro = CapturarPalabra(reader.password, 36, 16, PASSWORD, 1);
        ...
        if (salirRegistro != ESC){
            CapturarPalabra(validatePassword, 36, 18, PASSWORD, 1);
        ...
        case TAB: // Tecla TAB
            if (validos > 0){
                ...
                case 0:
                    ...
                    do {
                      teclaBusqueda = CapturarPalabra(filtro, 7, 6, 30, 0);
        ...
        case F1: // F1 Perfil
            ...
            do
            {
                ...
                case F1:
                    ...
                    char newPhone[PHONE];
                    int ultimaTecla = 0;
                    ultimaTecla = CapturarPalabra(newPhone,36,10,PHONE,0);
                ...
                case F2:
                    ...
                    ultimaTecla = 0;
                    ultimaTecla = CapturarPalabra(newAddress,36,10,ADDRESS,0);
...
int CambiarPassword(stReader* reader){
    ...
    {
        teclaCaptura = CapturarPalabra(passwordVieja,36,14,PASSWORD,1);
        if (teclaCaptura != ESC)
        {
    ...
    {
        CapturarPalabra(passwordNueva,36,16,PASSWORD,1);
        teclaCaptura = CapturarPalabra(passwordValidacion,36,18,PASSWORD,1);
        if (teclaCaptura != ESC)
        {
...
void LoginLector(const char fileName[]){
    ...
    hidecursor(1);
    do {
        teclaUserName = CapturarPalabra(username, 24, 10, MAIL, 0); // el ultimo parametro es para saber si tiene que escribir el caracter o *
        if (teclaUserName == ENTER){
            teclaPassword = CapturarPalabra(password, 24, 12, PASSWORD, 1);
        ...
        case ESC:
            if (hizoCambios == 1){
                ...
                do{
                    ...
                    if (salir == SMAY || salir == SMIN){
                        // Guardar cambios del reader
                        GuardarLectores(archivoLectores, readers, validos);
                    }
                }
                while (salir == 0);
            }
    ...
    if (teclaModificarLector != ESC){
        ...
        if (teclaModificarLector != ESC){
            ...
            if (teclaModificarLector != ESC){
                ...
                if (teclaModificarLector !=ESC){
                    teclaModificarLector = CapturarPalabra(palabra, 17, 11, PASSWORD, 0);
