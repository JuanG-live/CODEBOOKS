...
int BuscarLibroNombreMenor(stBook books[], int validos, int posicionInicial, int campo){
    ...
    if (campo == 0) {
        ...
        while (i < validos){
            if (strcmp(books[i].name, nombre) < 0){
                strcpy(nombre, books[i].name);
                posicionMenor = i;
            }
            i++;
...
int CrearNuevoLibro(stBook* bookAux){
    ...
    MenuNuevoLibro();
    do{
        teclaNuevoLibro = CapturarPalabra(bookAux->name, 17, 6, 30, 0);
    ...
    if (teclaNuevoLibro != ESC){
        do{
            teclaNuevoLibro = CapturarPalabra(bookAux->author, 17, 7, 30, 0);
    ...
    if (teclaNuevoLibro != ESC){
        do{
            teclaNuevoLibro = CapturarPalabra(bookAux->editorial, 17, 8, 30, 0);
    ...
    if (teclaNuevoLibro != ESC){
        do{
            teclaNuevoLibro = CapturarPalabra(bookAux->gender, 17, 9, 20, 0);
    ...
    if (teclaNuevoLibro != ESC){
        do{
            teclaNuevoLibro = CapturarPalabra(bookAux->ISBN, 17, 10, 20, 0);
    ...
    if (teclaNuevoLibro != ESC){
        do{
            teclaNuevoLibro = CapturarPalabra(bookAux->description, 17, 11, 200, 0);
...
void AdministrateBooks(const char archivoLibros[]){
    ...
    do{
        ...
        case F4: // ORDENAR
            ...
            break;
        case F5: // GUARDAR
            resultado = GuardarLibros(archivoLibros, books, validos);
            if (resultado > 0){
